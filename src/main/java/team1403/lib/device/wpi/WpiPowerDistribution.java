...
public class WpiPowerDistribution extends PowerDistribution
   ...
   */
  public WpiPowerDistribution(String name, CougarLogger logger) {
    m_name = name;
    m_logger = logger;
  }
  ...
  public CurrentSensor makeCurrentSensor(String name, int channel) {
    if (channel < 0 || channel >= 24) {
      m_logger.errorf("Invalid circuit channel=", channel);
    ...
    m_logger.infof("Registering circuit %d as '%s'", channel, name);
