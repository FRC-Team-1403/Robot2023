...
//Taken from https://github.com/BroncBotz3481/YAGSL/blob/main/swervelib/kinematics/SwerveDriveKinematics2.java

/**
 * Clone of WPI SwerveKinematics, which implements second order kinematics when
 * calculating modules states from chassis speed.
 * Makes use of {@link SwerveDriveKinematics2} to add the angular velocity that
 * is required of the module as an output.
 */
public class SwerveDriveKinematics2 extends edu.wpi.first.math.kinematics.SwerveDriveKinematics {

  private final SimpleMatrix m_inverseKinematics;
  private final SimpleMatrix m_forwardKinematics;
  private final SimpleMatrix m_bigInverseKinematics;
   ...
   * of wheel locations as
   * {@link Translation2d}s. The order in which you pass in the wheel locations is
   * the same order that you will receive
   * the module states when performing inverse kinematics. It is also expected
   * that you pass in the module states in the
  ...
  public SwerveDriveKinematics2(Translation2d... wheelsMeters) {
    ...
    m_bigInverseKinematics = new SimpleMatrix(m_numModules * 2, 4);
    ...
    for (int i = 0; i < m_numModules; i++) {
      ...
      m_bigInverseKinematics.setRow(i * 2, 0, /* Start Data */ 1, 
          0, -m_modules[i].getX(), -m_modules[i].getY());
      m_bigInverseKinematics.setRow(i * 2 + 1, 0, /* Start Data */ 0, 1, 
          -m_modules[i].getY(), +m_modules[i].getX());
    }
  ...
  public static void desaturateWheelSpeeds(
    ...
    double translationalK = Math.hypot(currentChassisSpeed.vxMetersPerSecond, 
  ...
  public SwerveModuleState2[] toSwerveModuleStates(
    ...
    var moduleAccelerationStatesMatrix = m_bigInverseKinematics.mult(accelerationVector);
